...
function renderAllDep() {
  ...
  if ($('adminFlightsListDep')) {
    if (!adminFlights.length) {
      ...
      $('adminFlightsListDep').innerHTML = adminFlights.map(f => `
        <div class="admin-row"><div class="admin-row-info"><span class="admin-row-number">${f.flightNumber}</span><span class="admin-row-route">${f.destination} (${f.iataCode||''})</span><span class="status-tag ${getTagClass(f)}" style="font-size:10px;">${(f.statusText||'').replace(/\n/g,' ')}</span></div><div class="admin-row-actions"><button class="btn-icon" onclick="event.stopPropagation();editFlightDep('${f.id}')"><i class="fas fa-pen"></i></button><button class="btn-icon danger" onclick="event.stopPropagation();deleteFlightDep('${f.id}')"><i class="fas fa-trash"></i></button></div></div>
      `).join('');
    }
  }
  
  // Используем flightDay от сервера
  if ($('flightsYesterdayDep')) {
    const arr = flightsDep.filter(f => f.flightDay === 'yesterday' || (!f.flightDay && getFlightDayLocal(f) === 'yesterday'));
    $('flightsYesterdayDep').innerHTML = arr.length === 0 ? `<tr class="empty"><td colspan="6"><div class="empty-msg"><i class="fas fa-plane"></i><p>Нет рейсов за вчера</p></div></td></tr>` : arr.map(f => renderFlightRow(f, true)).join('');
  }
  
  if ($('flightsTodayDep')) {
    const arr = flightsDep.filter(f => {
      if (f.status === 'departed' || f.status === 'early_departed') return showDepartedDep;
      const day = f.flightDay || getFlightDayLocal(f);
      return day === 'today';
    });
    $('flightsTodayDep').innerHTML = arr.length === 0 ? `<tr class="empty"><td colspan="6"><div class="empty-msg"><i class="fas fa-plane"></i><p>Нет рейсов на сегодня</p></div></td></tr>` : arr.map(f => renderFlightRow(f, showDepartedDep)).join('');
  }
  ...
  if ($('flightsTomorrowDep')) {
    ...
    $('flightsTomorrowDep').innerHTML = arr.length === 0 ? `<tr class="empty"><td colspan="6"><div class="empty-msg"><i class="fas fa-plane"></i><p>Нет рейсов на завтра</p></div></td></tr>` : arr.map(f => renderFlightRow(f, false)).join('');
  }
}

function getFlightDayLocal(f) {
  const d = f.expectedDeparture ? new Date(f.expectedDeparture) : f.scheduledDeparture ? new Date(f.scheduledDeparture) : null;
  if (!d || isNaN(d.getTime())) return 'today';
  const now = getLocalNow();
  const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 0, 0, 0);
  const tomorrowStart = new Date(todayStart.getTime() + 86400000);
  if (d >= tomorrowStart) return 'tomorrow';
...
function renderAllArr() {
  ...
  if ($('flightsYesterdayArr')) {
    const arr = flightsArr.filter(f => f.flightDay === 'yesterday' || (!f.flightDay && getFlightDayLocal(f) === 'yesterday'));
    $('flightsYesterdayArr').innerHTML = arr.length === 0 ? `<tr class="empty"><td colspan="6"><div class="empty-msg"><i class="fas fa-plane-arrival"></i><p>Нет рейсов за вчера</p></div></td></tr>` : arr.map(f => renderFlightRowArr(f, true)).join('');
  }
  if ($('flightsTodayArr')) {
    const arr = flightsArr.filter(f => { if (f.status === 'arrived') return showDepartedArr; const day = f.flightDay || getFlightDayLocal(f); return day === 'today'; });
    $('flightsTodayArr').innerHTML = arr.length === 0 ? `<tr class="empty"><td colspan="6"><div class="empty-msg"><i class="fas fa-plane-arrival"></i><p>Нет рейсов на сегодня</p></div></td></tr>` : arr.map(f => renderFlightRowArr(f, showDepartedArr)).join('');
  }
  if ($('flightsTomorrowArr')) {
    const arr = flightsArr.filter(f => { if (f.status === 'arrived') return false; const day = f.flightDay || getFlightDayLocal(f); return day === 'tomorrow'; });
    $('flightsTomorrowArr').innerHTML = arr.length === 0 ? `<tr class="empty"><td colspan="6"><div class="empty-msg"><i class="fas fa-plane-arrival"></i><p>Нет рейсов на завтра</p></div></td></tr>` : arr.map(f => renderFlightRowArr(f, false)).join('');
  }
}
