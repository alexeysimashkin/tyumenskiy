...
app.get('/api/flights', async (req, res) => {
  ...
  if (cleaned.length !== flights.length) {
    for (const f of flights) {
      if (!cleaned.includes(f)) await deleteOne(f.id, table);
    }
    flights = cleaned;
  }
  ...
  flights = flights.map(f => ({ 
    ...f, 
    computedStatus: type === 'departure' ? computeStatus(f) : computeArrivalStatus(f), 
    statusText: type === 'departure' ? getStatusText(f) : getArrivalStatusText(f),
